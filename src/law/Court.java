...
Main.currentCase.clearSprites();	// removes sprites
	...
	if(x==2) { // checks if view is of the prosecutor
		...
		if (Main.currentCase.getCharacter(Main.currentCase.currentProsecutor)==null) Main.currentCase.characters.add(new Character("Main.currentCase.currentProsecutor", 100, 135)); // adds character
		Main.renderer.queue[1]= new Drawable(1, 300,280+68,1,0,1); // draws bench
	}	
